import { initAuth, signInWithGoogle, getAuthState } from './supabase.js';
import { createSavedFilter, createTask, deleteSavedFilter, loadWorkspace, persistLocalTasks, searchWorkspaceTasks, subscribeToWorkspace, updateTask } from './data.js';
...
function renderTimeline() {
  ...
  $('#timeline').innerHTML = times.map(time => {
    const task = todayTasks.find(item => item.time === time);
    return `<div class="timeline-row">
      <span class="time-label">${time}</span>
      <div class="timeline-content">${task ? `<article class="timeline-task ${task.color}-line ${task.status === 'done' ? 'completed' : ''}">
        ${taskCheck(task)}
        <div><strong>${task.title}</strong><p>${task.project}${task.meeting ? ' · Google Meet' : ''}</p></div>
        <span class="task-duration">${task.duration}</span>
      </article>` : `<button class="empty-slot" data-open-task>+ Schedule something here</button>`}</div>
    </div>`;
  }).join('');
}
...
function renderCalendar() {
  ...
  hours.forEach((hour, row) => {
    ...
    days.forEach((day, col) => {
      const events = weekOffset === 0 ? getCalendarEvent(row, col) : '';
...
function getCalendarEvent(row, col) {
  const events = {
...
function updateThemeButton(theme) {
  ...
  document.querySelector('meta[name="theme-color"]').content = theme === 'dark' ? '#101113' : '#f6f6f3';
}
...
$('#syncButton').onclick = async function () {
  ...
  $('.sync-dot', this).style.background = 'var(--yellow)';
  setTimeout(() => { this.classList.remove('syncing'); $('.sync-dot', this).style.background = ''; showToast('Google Calendar is connected'); }, 1100);
};
...
$('#prevWeek').onclick = () => { weekOffset--; renderCalendar(); };
$('#nextWeek').onclick = () => { weekOffset++; renderCalendar(); };
$('#calendarTodayButton').onclick = () => { weekOffset = 0; renderCalendar(); };
$('#planDayButton').onclick = () => { switchView('calendar'); showToast('Drag tasks into open time'); };
...
$('#taskForm').addEventListener('submit', async event => {
  ...
  tasks.unshift(task); save(); renderAll(); closeTaskModal(); event.target.reset(); $('#scheduleSwitch').classList.remove('on'); showToast('Task added');
  try {
...
document.addEventListener('keydown', event => {
  ...
  if (event.key === 'Escape') { closeSearch(); $('.sidebar').classList.remove('open'); }
});
...
initAuth({
  onReady: async ({ configured, user, error }) => {
    ...
    $('.avatar').textContent = fullName.split(' ').map(part => part[0]).join('').slice(0, 2).toUpperCase();
